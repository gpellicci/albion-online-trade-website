   			...
   			success: function(data) { 
   				...
   				if(!$.isArray(data)){
	   				...
	   				data.forEach(function (item, index){
	   					buildAnnounce(item, index, item.seller_discord_id);   					
	   				});
...
function buildAnnounce(item, index, id){
	...
	myAnnounce = "<div class='row justify-content-center'><div class='col-md-5 col-lg-4'><div class='clean-pricing-item'><div class='heading'><h3>"+ item.item_localized_name + "</h3></div><div><img style='display: none;' src='https://gameinfo.albiononline.com/api/gameinfo/items/"+itempicURL+"' class='center'></div>"/*<p>"+item.item_description+"<br></p>*/+"<div class='features'><h4><span class='feature'>Quantity:&nbsp;</span><span>"+item.item_quantity+"</span></h4><h4><span class='feature'>Time left:&nbsp;</span><span>" + parseTime(2678400 - item.announce_age) + "</span></h4></div><div class='price'><h4>"+ getNumberWithCommas(item.item_price) +" silver</h4></div><button class='btn btn-outline-primary btn-block contact-seller-now' type='button' value='"+index+"'>CONTACT SELLER NOW <img style='vertical-align: text-top; width: 20px;' id='preview" + index + "'></img> </button></div></div></div>";
