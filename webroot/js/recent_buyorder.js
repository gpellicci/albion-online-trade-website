$(document).ready(function () {
	$.ajax({
		type: 'POST',
		url: 'https://albion-trade.com/php/recent_buyorder.php',
...
function buildRow(item, index, element){
	...
	myRow = "<tr id='"+index+"'><td>"+ replaceTier(item.item_localized_name, item.item_enchant, item.item_quality) + "</td><td style='text-align:center!important'>"+item.item_quantity+"</td><td>"+item.announce_birth+"</td><td>"+getNumberWithCommas(item.item_price)+"</td><td>"+item.city+"</td><td><img style='height: 35%;'src='https://gameinfo.albiononline.com/api/gameinfo/items/"+itempicURL+"' class='center'></td><td><button class='btn btn-outline-primary btn-block contact-seller-now' type='button' value='"+index+"'>CONTACT BUYER NOW</button></td></tr>";
