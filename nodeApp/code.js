...
app.post('/pog', (req, res) => {
  ...
  if(wts_member == null || wtb_member == null){
    ...
    msg = "-----\n"+ new Date() +"\nwtb: "+wtb+"    wts: "+wts+"  ||  Item: **T" + info.tier + "." + info.enchant + "** "+ mapQuality(info.quality) + " " + info.item+"\n-----\n\n";
    fs.appendFile(path.resolve(__dirname, 'sell-log.txt'), msg, function (err) {
...
app.post('/buyorder', (req, res) => {
  ...
  if(wts_member == null || wtb_member == null){
    ...
    msg = "-----\n"+ new Date() +"\n--BUY ORDER; wtb: "+wtb+"    wts: "+wts+"  ||  Item: **T" + info.tier + "." + info.enchant + "** "+ mapQuality(info.quality) + " " + info.item+"\n-----\n\n";

    fs.appendFile(path.resolve(__dirname, 'buyorders-log.txt'), msg, function (err) {
      
    });
    console.log(msg);
    string = "<@" + wts + ">  is interested in <@"+wtb+"> buy order!   ||||  Item: **T" + info.tier + "." + info.enchant + "** "+ mapQuality(info.quality) + " " + info.item;
    channel.send(string); //print msg in general chat
    //memberStatus(members);      //print online/dnd/away/offline status    
    console.log(info.item);
    res.status(200).send("success");
